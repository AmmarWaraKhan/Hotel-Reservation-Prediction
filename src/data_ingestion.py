...
class DataIngestion:
    def __init__(self, config):
        ...
        self.train_teest_ratio = self.config["train_ratio"]
    ...
    def download_csv_from_gcp(self):
        try:
            ...
            logger.info(f"CSV File is sucessfully dowanloaded to {RAW_FILE_PATH}")
    ...
    def split_data(self):
        try:
            ...
            train_data, test_data = train_test_split(data, test_size=1 - self.train_teest_ratio, random_state=42)
            
            train_data.to_csv(TRAIN_FILE_PATH, index=False)
            test_data.to_csv(TEST_FILE_PATH, index=False)
            
            logger.info(f"Data splitting completed. Train data saved to {TRAIN_FILE_PATH}")
            logger.info(f"Data splitting completed. Test data saved to {TEST_FILE_PATH}")
